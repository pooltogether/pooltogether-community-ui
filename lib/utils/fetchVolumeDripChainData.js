...
const fetchTokenForDrip = async (array, networkName, comptrollerAddress, dripAddress) => {
  try {
    const tokenChainValues = await fetchTokenChainData(networkName, comptrollerAddress, dripAddress)
...
export const fetchVolumeDripChainValues = async (
  ...
  try {
    ...
    const values = await batch(
      provider,
      etherplexComptrollerContract.getActiveVolumeDripTokens(
        prizeStrategyAddress,
        ticketAddress,
        !isReferrer
      ),
      etherplexComptrollerForReferralVolumeDripsContract.getActiveVolumeDripTokens(
        prizeStrategyAddress,
        ticketAddress,
        isReferrer
      )
    ...
    const activeReferralVolumeDrips =
      values.comptrollerForReferralVolumeDrips.getActiveVolumeDripTokens[0]

    let drips = []
    activeVolumeDrips.forEach(async (dripAddress) =>
      fetchTokenForDrip(drips, networkName, comptrollerAddress, dripAddress)
    )

    let referralDrips = []
    activeReferralVolumeDrips.forEach(async (dripAddress) =>
      fetchTokenForDrip(referralDrips, networkName, comptrollerAddress, dripAddress)
    )
