...
export const fetchTokenChainValues = async (provider, usersAddress, erc20ContractAddress) => {
  try {
    const etherplexTokenContract = contract('token', ERC20Abi, erc20ContractAddress)

    const values = await batch(
      provider,
      etherplexTokenContract.decimals().name().symbol().balanceOf(usersAddress)
    )
...
export const fetchTokenChainData = async (networkName, usersAddress, erc20ContractAddress) => {
