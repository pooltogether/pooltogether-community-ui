...
export const StartAwardUI = (props) => {
  ...
  const handleClick = (e) => {
    ...
    handleStartAwardSubmit(setTx, provider, props.poolAddresses.prizeStrategy)
  }
    ...
    <>
      {!canStartAward && !isRngRequested && !canCompleteAward && (
        <>
          <div className='my-4'>
            ...
            <div className='font-bold'>Open for deposits ...</div>
          </div>
        </>
      )}
      ...
      {!txInFlight ? (
        <>
          {canStartAward && (
            <>
              <Button onClick={handleClick} color='blue' size='sm'>
                Start award
              </Button>
            </>
          )}
