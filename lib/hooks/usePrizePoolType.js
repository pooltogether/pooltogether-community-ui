  ...
  useEffect(() => {
    ...
    const determinePoolType = async () => {
      ...
      try {
        ...
        //     error: true,
        //     errorMessage: ``,
        //     view: <IncompatibleVersion prizePool={prizePool} setPrizePoolType={setPrizePoolType} />
        //   })
        //   return
        // }
      } catch (e) {
...
const IncompatibleVersion = (props) => {
  const { prizePool, setPrizePoolType } = props
  const [errorState, setErrorState] = useAtom(errorStateAtom)
  const [network] = useAtom(networkAtom)
  ...
  return (
    <>
      ...
      <div className='flex flex-col'>
        <Button className='my-4' color='secondary' href={`/pools/mainnet/${prizePool}`}>
          Mainnet
        </Button>
        <Button className='my-4' color='secondary' href={`/pools/rinkeby/${prizePool}`}>
          Rinkeby
        </Button>
      </div>
      {/* TODO: It'd be nice to have this override work */}
