...
export const usePrizePoolContracts = () => {
  ...
  const enabled = isReadProviderReady && Boolean(prizePoolAddress) && poolAddressIsValid

  return useQuery(
    [QUERY_KEYS.contractVersions, chainId, prizePoolAddress],
    async () =>
      await _fetchPrizePoolContracts(readProvider, chainId, prizePoolAddress, setErrorState),
    // @ts-ignore
    {
      ...NO_REFETCH_QUERY_OPTIONS,
      enabled,
...
const _fetchPrizeAndStrategyContractVersions = async (
  ...
  let prizePoolVersion = {
    ...
    contract: CONTRACTS.stake,
    version: '3.2.0'
  }

  // Handle special case for this Aave USDT pool released on Polygon:
  if (prizePoolAddress.toLowerCase() === '0x887e17d791dcb44bfdda3023d26f7a04ca9c7ef4') {
    prizePoolVersion.contract = CONTRACTS.yield
  }
  ...
  let prizeStrategyVersion = {
    ...
    contract: CONTRACTS.multipleWinners,
    version: '3.2.0'
  }
