...
import { errorStateAtom, poolAddressesAtom, prizePoolTypeAtom } from 'lib/components/PoolUI'
...
export const usePoolAddresses = (provider) => {
  ...
  useEffect(() => {
    ...
    const fetchPoolAddresses = async () => {
      if (prizePool && !ticket) {
        try {
          if (poolType === PRIZE_POOL_TYPE.compound) {
            // Query Prize Pool
          ...
          } else if (poolType === PRIZE_POOL_TYPE.stake) {
            // Query Prize Pool
