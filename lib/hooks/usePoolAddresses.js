...
const _getPoolAddress = (poolAlias) => {
  switch (poolAlias) {
    case 'rinkeby-test': {
      return '0x4706856FA8Bb747D50b4EF8547FE51Ab5Edc4Ac2'
    }
    case 'basis': {
      return '0x1Ee6194b0e28C12dF0FD107e5eb883Aa071F8C9a'
    }
    default: {
      throw new Error('no matching poolAlias data found for: ', poolAlias)
    }
  }
}
...
export const usePoolAddresses = () => {
  ...
  const prizePoolAddress = useMemo(() => {
    return poolAlias ? _getPoolAddress(poolAlias) : router.query.prizePoolAddress
  }, [
  ...
  }, [poolAlias, prizePoolAddress, network.id])
