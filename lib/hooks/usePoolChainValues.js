...
import { getDataFetchingErrorMessage } from 'lib/components/PoolData'
import {
  CONTRACTS,
  CONTRACT_ADDRESSES,
...
export const usePoolChainValues = () => {
  ...
  return useQuery(
    ...
    async () =>
      _fetchPoolChainValues(provider, chainId, prizePoolContracts, prizePoolType, setErrorState),
    {
      enabled,
...
export const _fetchPoolChainValues = async (
  ...
  try {
    ...
    setErrorState({
      error: true,
      errorMessage: getDataFetchingErrorMessage(prizePoolAddress, 'pool chain values', e.message)
    })
...
const formatChainData = (
  ...
  return {
    config: {
      numberOfWinners: prizeStrategy.numberOfWinners[0].toNumber(),
    ...
    sponsorship: {
      address: prizePoolContracts.sponsorship,
    ...
    ticket: {
      address: prizePoolContracts.ticket,
      decimals: ticket.decimals[0] || DEFAULT_TOKEN_PRECISION,
    ...
    token: {
      address: prizePoolContracts.token,
