...
import { poolAddressesAtom, poolChainValuesAtom, prizePoolTypeAtom } from 'lib/components/PoolUI'
...
export const usePoolChainValues = () => {
  ...
  useEffect(() => {
    ...
    const fetchPoolChainValues = async () => {
      const { prizeStrategy, ticket, sponsorship, token, cToken, prizePool } = poolAddresses
      console.log('fetchPoolChainValues', poolAddresses)
      if (provider && prizeStrategy && ticket && sponsorship && prizePool) {
        try {
          ...
          if (prizePoolType === PrizePoolType.compound) {
